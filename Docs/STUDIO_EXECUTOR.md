...
- **Source Boundary**: `sourceSha` is the exact immutable commit being tested and must be reachable from the requested canonical `sourceRef`; unrelated or missing revisions fail before Studio execution. The canonical ref may advance after a request is queued without invalidating an ancestor pin.
...
Resolve the exact canonical `main` SHA immediately before creating the request and pin that immutable commit. If `main` advances while the request is queued, the pin remains valid only while that exact SHA is still reachable from the named canonical ref. The value below is intentionally shown as a placeholder because a hard-coded example would quickly become historical.
...
```json
{
  ...
  "sourceRef": "main",
  "sourceSha": "<exact-40-char-pinned-main-sha>",
...
The production request validator also rejects duplicate result IDs, unsafe refs/strings, unsupported keys, unsupported job types, and a `sourceSha` that is not reachable from the canonical remote ref.
...
1. the result targets the intended exact pinned source SHA,
