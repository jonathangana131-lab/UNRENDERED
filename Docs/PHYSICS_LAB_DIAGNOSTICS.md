...
Before a readout is accepted, the realized representation's fidelity, `UNRENDERED_RecipeKey`, `UNRENDERED_RepresentationRevision`, and `UNRENDERED_StateRevision` attributes must agree exactly with the authoritative `WorldEntity` record. Revision attributes must be valid nonnegative integers. The identity-tagged realized representation population must also equal the production realizer's `UNRENDERED_RepresentedCount`, every direct child must be the Realizer's exact privately owned representation for that identity, and duplicate identities are rejected. Diagnostics fails closed on missing, malformed, stale, contradictory, substituted, or incomplete physical metadata instead of projecting a partial evidence surface.

When diagnostics are enabled, machine-readable capture validates the owned visual tree itself. There must be exactly one uniquely named `BillboardGui` per represented entity, each billboard must target that entity's current represented `BasePart`, each billboard must contain exactly one `Readout` `TextLabel`, the label text must exactly match the current authoritative identity/fidelity/revisions plus region fingerprint/repro key, and the total owned descendant count must match the one-billboard/one-label shape. A deleted, extra, duplicated, renamed, retargeted, stale, or malformed live readout therefore invalidates the snapshot instead of allowing a complete entity list to mask a broken visual overlay.
...
4. the machine-readable snapshot agrees with production `WorldEntity` state; every diagnosed direct child is the exact Realizer-owned representation; the realized fidelity/recipe/revision attributes agree with authoritative state; the identity-tagged representation population matches `UNRENDERED_RepresentedCount`; and the owned visual tree has exactly one correctly targeted, current-text billboard/readout pair per represented entity;
