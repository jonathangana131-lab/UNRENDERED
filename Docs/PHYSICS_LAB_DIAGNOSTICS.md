...
When diagnostics are enabled, machine-readable capture also validates the owned visual tree itself: there must be exactly one expected `BillboardGui` per represented entity, each must contain exactly one `Readout` `TextLabel`, and the total owned descendant count must match that one-billboard/one-label shape. It then verifies that each billboard's `Adornee` is the exact `BasePart` selected from the same authoritative representation and that each label's `Text` exactly matches the shared deterministic formatter for that entity's identity, fidelity, representation/state revisions, resolved-region fingerprint, and repro key. A deleted, extra, renamed, malformed, redirected, or text-mutated live readout therefore invalidates the snapshot instead of allowing a complete machine-readable entity list to mask false visual evidence.
...
4. the machine-readable snapshot agrees with production `WorldEntity` state; every enumerated identity-tagged child is the exact direct representation owned by the production Realizer for that identity; the realized fidelity/recipe/revision attributes agree with that same authoritative state; the duplicate-free representation population matches `UNRENDERED_RepresentedCount`; and the owned visual tree contains exactly one billboard/readout pair per represented entity whose `Adornee` and visible text exactly agree with that same authoritative representation/evidence identity;
