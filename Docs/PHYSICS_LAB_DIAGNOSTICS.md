...
Before a readout is accepted, the realized representation's fidelity, `UNRENDERED_RepresentationRevision`, and `UNRENDERED_StateRevision` attributes must agree exactly with the authoritative `WorldEntity` record. Revision attributes must be finite nonnegative integers. Diagnostics fails closed on missing, malformed, or stale revision metadata instead of showing authoritative values over a contradictory realized representation.
...
4. the machine-readable snapshot agrees with production `WorldEntity` state, and the realized fidelity/revision attributes agree with that same authoritative state;
5. diagnostics-off leaves no owned diagnostics tree;
6. the 20-cycle sweep records zero scoped resource drift and stable full-lab envelope at required checkpoints, and its final post-yield observation is still OFF with zero owned diagnostics Instances;
