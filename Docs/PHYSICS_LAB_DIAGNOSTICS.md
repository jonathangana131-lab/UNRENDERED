...
`runToggleEvidence()` validates the cycle request before any evidence mutation or iteration. Exact `nil` selects the canonical twenty-cycle proof; explicit overrides are accepted only as integer values from 1 through 20, so a runtime-typed caller cannot expand the Studio-only full-validation loop into an unbounded workload.
