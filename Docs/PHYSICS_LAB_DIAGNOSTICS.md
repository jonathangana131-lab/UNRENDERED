...
`runToggleEvidence()` first validates its cycle request before any evidence mutation or iteration. Exact `nil` selects the canonical twenty-cycle proof; explicit overrides are accepted only as integer values from 1 through 20, so the Studio-only full-validation loop cannot be expanded into an unbounded workload through a runtime-typed caller.
