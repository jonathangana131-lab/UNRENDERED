...
`runToggleEvidence()` validates its cycle request before any evidence iteration. Exact `nil` selects the canonical twenty-cycle proof; an explicit override must be a finite integer from 1 through 20. Larger values are rejected rather than expanding this Studio-only full-validation helper into an unbounded workload.
