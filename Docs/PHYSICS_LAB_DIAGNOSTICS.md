...
The helper yields a real `RunService.Heartbeat` before each enable and again after each completed cycle so the full canonical-validation soak does not become one uninterrupted Studio script timeslice. Those yields are scheduling only: they do not relax the diagnostics-on checks, diagnostics-off resource deltas, or envelope assertions.

A failed sweep is not reusable evidence. Current source disables the owned diagnostics overlay before each cycle's full canonical `PhysicsLabValidation.captureFull()`/`compare()` pass, and `enable()` rolls back the exact folder it created when its protected realization/capture transaction fails. Those known contract failures therefore leave diagnostics off. Hard process termination or an unexpected failure outside the protected paths is not proven cleanup; before any retry or other candidate observation, explicitly run `Diagnostics.disable(lab)` from the server Command Bar and verify `Diagnostics.capture(lab).enabled == false`. Do not reuse partial checkpoints from a failed sweep.
