...
Any other canonical domain that retains a material assignment must preserve the exact plain-data recipe reference: semantic `id`, `schemaVersion`, and `recipeRevision`. The semantic ID alone is not sufficient durable truth because a later content revision may intentionally keep that ID while changing material values. This reference rule does not expose Roblox assets or renderer/package details to domain data.
