...
- Request `20260810-032-performance-observation-090e-sol56s9` on exact UNRENDERED SHA `090e112c83126ebe8e6f9ba75f27b7bcc31dc3af` is accepted bounded `physics-lab-performance-observation` evidence. The stored result is typed/raw `PASS` and records real Studio server RunMode on Roblox engine `0.733.0.7330989`, 30 warmup + 120 measured Heartbeats (mean 17.785 ms, median 16.692 ms, p95 18.304 ms, max 93.643 ms), full-capture/restart observations, exact canonical WorldId/RegionId/fingerprint/repro identity, zero tracked restart resource deltas, and the documented 0.001-stud envelope preserved. After the original evaluator review, bridge #65 hardened total/mean/resource-delta checks and bridge #75 added exact nearest-rank percentile/extrema feasibility; bridge #78 then durably loaded the complete stored result and passed its full evidence object through that current hardened evaluator, with exact-head Bridge CI `31450117146` green and reason `performance observations recorded; no budget asserted`. This accepts only the bounded measurement row: **no permanent frame-time, startup, or device-suitability budget is asserted**, and it does not close the composite Hero Gate. Request `20260810-033-performance-post65-090e-sol56h3` later timed out before producing evidence and adds no gate credit.
...
- The bridge now has accepted lifecycle/rebuild, single-server bootstrap/baseline, canonical F2-shell physical-sanity/contact, and bounded performance-observation evidence, but #151 still lacks accepted diagnostics and true two-client authority evidence.
...
2. keep source-owned lifecycle/repro/ownership, accepted physical-sanity, and accepted bounded performance evidence green, and keep every remaining evidence path fail closed on regressions,
