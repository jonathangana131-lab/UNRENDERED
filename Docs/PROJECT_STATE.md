...
- Request `20260810-032-performance-observation-090e-sol56s9` on exact UNRENDERED SHA `090e112c83126ebe8e6f9ba75f27b7bcc31dc3af` is durable `physics-lab-performance-observation` `PASS` / `performance observations recorded; no budget asserted`: real Studio server RunMode on Roblox engine `0.733.0.7330989`, 30 warmup + 120 measured Heartbeats (mean 17.785 ms, median 16.692 ms, p95 18.304 ms, max 93.643 ms), full-capture/restart observations, exact canonical WorldId/RegionId/fingerprint/repro identity, zero tracked restart resource deltas, and the documented 0.001-stud envelope preserved. The later stricter bridge evaluator also rejects contradictory aggregate timing/resource payloads, and this recorded payload satisfies those consistency conditions. This accepts the requested measurement-observation row only; it does **not** establish a permanent frame-time/startup/device-suitability budget, close the composite Hero Gate, or unlock a feature.
