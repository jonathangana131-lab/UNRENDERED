...
- Request `20260810-032-performance-observation-090e-sol56s9` on exact UNRENDERED SHA `090e112c83126ebe8e6f9ba75f27b7bcc31dc3af` is durable `physics-lab-performance-observation` `PASS` / `performance observations recorded; no budget asserted`: real Studio server RunMode on Roblox engine `0.733.0.7330989`, 30 warmup + 120 measured Heartbeats (mean 17.785 ms, median 16.692 ms, p95 18.304 ms, max 93.643 ms), full-capture/restart observations, exact canonical WorldId/RegionId/fingerprint/repro identity, zero tracked restart resource deltas, and the documented 0.001-stud envelope preserved. Bridge #65 and #75 subsequently hardened aggregate/resource validation, and bridge #78 (`d9805bbf24d865dc6286d70a813a936b7ff0d123`) durably loaded the complete stored request-032 result and passed its full evidence payload through the current hardened evaluator with exact-head Bridge CI `31450117146` green. This satisfies the required hardened-evaluator re-evaluation for the bounded measurement-observation row. A separate fresh post-#65 Studio rerun (`20260810-033-performance-post65-090e-sol56h3`) timed out without evidence and is not counted. No permanent frame-time/startup/device-suitability budget is asserted, and this row does not close the composite Hero Gate or unlock a feature.
...
2. keep source-owned lifecycle/repro/ownership, accepted physical-sanity, and measurement-observation evidence green and fail closed on regressions,
