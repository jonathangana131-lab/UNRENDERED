...
- The bridge now has accepted lifecycle/rebuild, single-server bootstrap/baseline, canonical F2-shell physical-sanity/contact, and bounded measurement-only performance-observation evidence, but #151 still lacks accepted diagnostics and true two-client authority evidence. The first post-#97 exact-SHA probes for both remaining rows failed closed with `NO_CAPTURE` and empty evidence, so the runner-side GUI/display path remains a practical external blocker rather than evidence of a gameplay/source PASS.
