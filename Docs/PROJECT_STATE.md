...
- Request `20260810-032-performance-observation-090e-sol56s9` on exact UNRENDERED SHA `090e112c83126ebe8e6f9ba75f27b7bcc31dc3af` recorded a useful pre-hardening `physics-lab-performance-observation` payload: real Studio server RunMode on Roblox engine `0.733.0.7330989`, 30 warmup + 120 measured Heartbeats (mean 17.785 ms, median 16.692 ms, p95 18.304 ms, max 93.643 ms), full-capture/restart observations, exact canonical WorldId/RegionId/fingerprint/repro identity, zero tracked restart resource deltas, and the documented 0.001-stud envelope preserved. Its then-current durable evaluator reported `PASS` / `performance observations recorded; no budget asserted`, but bridge #65 and #75 subsequently tightened fail-closed aggregate/resource validation and the integration guardrail requires fresh or re-evaluated exact-SHA evidence under the hardened evaluator before this row is accepted for #151. The hardened evaluator includes a positive regression proving request 032's recorded aggregate is numerically admissible; that regression is not a substitute for a durable post-hardening engine result. The performance row therefore remains **UNVERIFIED**. No permanent frame-time/startup/device-suitability budget is asserted.
- Future engine evidence requests must pin an exact canonical SHA that is reachable from the named canonical ref and satisfy the job-specific evaluator active for the evidence being consumed. No lifecycle, single-server, physical-sanity, or measurement-only performance result may unlock the Hero Gate by itself.
...
The source-contract portion of the Physics Lab gate is established. Accepted engine evidence currently covers the source-owned lifecycle/rebuild/resource-envelope slice, single-server bootstrap/baseline smoke, and the current F2 shell's canonical Studio physical-sanity/contact slice. Remaining engine-facing work includes accepted diagnostics capture/behavior evidence, true two-client canonical authority, and a performance-observation result consumed under the hardened fail-closed evaluator; articulated mechanisms remain UNVERIFIED where the current shell intentionally provides only anchored F2 proxies.
...
- The bridge has accepted lifecycle/rebuild, single-server bootstrap/baseline, and canonical F2-shell physical-sanity/contact evidence, but #151 still lacks accepted diagnostics, true two-client authority, and hardened-evaluator performance-observation evidence.
...
1. gather the remaining accepted diagnostics, true two-client canonical-authority, and hardened-evaluator performance-observation evidence without weakening source truth,
