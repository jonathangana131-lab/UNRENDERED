...
- Bridge #118 landed runner-owned non-PASS telemetry on bridge `main@fa8077e9365cd59e30a8c227397d8f056109558f` with Bridge CI `31455126625` green: bounded elapsed/timeout/watchdog state, Studio-alive-at-loop-end state, capture-attempt/success state, and request-bound marker counts are now durable failure diagnostics rather than inferred evidence. Those newly added runner-owned fields have not yet been exercised by a fresh post-#118 request. Earlier request `20260810-040-diagnostics-phaseprobe-835f-sol56sol`, published before #118 landed, targeted UNRENDERED `835f6bc39f9468a0fbe3c8f6c1c1249365fc9540` and published `FAIL` / `NO_CAPTURE` with `failureDiagnostics.phase=NO_REQUEST_BOUND_LOG`, `requestMarkerLines=0`, and zero foundation/lab/visual/result/finished markers. It sharpens the blocker to pre-request-bound Studio execution using the pre-#118 phase/marker diagnostics only; it is **not** accepted diagnostics evidence and does not justify another speculative retry while the display preflight is red.
...
- The bridge now has accepted lifecycle/rebuild, single-server bootstrap/baseline, canonical F2-shell physical-sanity/contact, and bounded measurement-only performance-observation evidence, but #151 still lacks accepted diagnostics and true two-client authority evidence. Post-#97 exact-SHA probes for the remaining rows have failed closed before request-bound Studio execution (`NO_CAPTURE`, empty evidence, zero request-bound markers). Bridge #118's newer runner-owned elapsed/watchdog/process/capture telemetry has not yet been exercised by a fresh request because speculative retries remain forbidden while the display preflight is red. The runner-side GUI/WindowServer/display path therefore remains the practical external blocker rather than evidence of a gameplay/source PASS.
