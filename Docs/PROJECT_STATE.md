...
- Post-#97 proof requests confirm a current external runner blocker rather than a gameplay/evaluator defect. Exact-SHA diagnostics request `20260810-036-diagnostics-displayguard-835f-sol56r8` validated and installed the managed plugin, then CoreGraphics reported no active macOS display across all five bounded wake/preflight checks; the bridge correctly refused a headless Studio launch and published durable `FAIL` / `NO_CAPTURE` / empty evidence. Fresh true-two-client request `20260810-039-two-client-post97-post93-835f-sol56m9` likewise published `FAIL` / `NO_CAPTURE` / empty evidence. Bridge issue #88 remains open until `UNRENDERED-STUDIO-MAC` is restored under a logged-in GUI/WindowServer session with at least one active display and one exact-SHA request reaches request-bound Studio execution. **Do not queue speculative diagnostics/two-client retries while this display preflight remains red.** These transport failures provide no #151 evidence credit and imply no UNRENDERED source regression.
...
- The self-hosted Studio runner currently has no CoreGraphics-active display/usable GUI WindowServer session. Bridge #88 is the external blocker; code-side wake/preflight/fail-fast behavior is already landed and verified fail-closed. Restore a logged-in macOS GUI session with at least one active display before queuing the next evidence request.
...
1. restore `UNRENDERED-STUDIO-MAC` under an active logged-in macOS GUI/display session, then gather the remaining accepted diagnostics and true two-client canonical-authority evidence without weakening source truth,
