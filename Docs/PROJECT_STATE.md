...
- Request `20260810-032-performance-observation-090e-sol56s9` on exact UNRENDERED SHA `090e112c83126ebe8e6f9ba75f27b7bcc31dc3af` is durable `physics-lab-performance-observation` `PASS` / `performance observations recorded; no budget asserted`: real Studio server RunMode on Roblox engine `0.733.0.7330989`, 30 warmup + 120 measured Heartbeats (mean 17.785 ms, median 16.692 ms, p95 18.304 ms, max 93.643 ms), full-capture/restart observations, exact canonical WorldId/RegionId/fingerprint/repro identity, zero tracked restart resource deltas, and the documented 0.001-stud envelope preserved. Bridge #65 hardened total/mean and integer-zero resource validation; bridge #75 added the exact nearest-rank p50/p95/extrema feasibility boundary; bridge #78 then durably loaded the **complete stored request-032 result**, pinned its request/job/source/exact SHA, and re-evaluated the full evidence payload through the current hardened evaluator with green Bridge CI (`31450117146`). That explicit re-evaluation satisfies the #151 integration guardrail and accepts this measurement-observation row. It does **not** establish a permanent frame-time/startup/device-suitability budget, rerun Studio, close the composite Hero Gate, or unlock a feature. Fresh post-#75 request `20260810-034-performance-rankbounds-090e-sol56s9` is additional corroboration, not a prerequisite for this accepted row.
...
- The bridge now has accepted lifecycle/rebuild, single-server bootstrap/baseline, canonical F2-shell physical-sanity/contact, and hardened/re-evaluated bounded performance-observation evidence, but #151 still lacks accepted diagnostics and true two-client authority evidence.
...
2. keep source-owned lifecycle/repro/ownership, accepted physical-sanity, and hardened performance-observation evidence green and fail closed on regressions,
