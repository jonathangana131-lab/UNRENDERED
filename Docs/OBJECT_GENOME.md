...
- `familyId + familyVersion + variantKey + schemaVersion` form the stable genome identity input. `ObjectGenome.identityParts()` is intended to feed the project StableId contract once that contract is locked; ObjectGenome does not duplicate hashing/canonical encoding.
...
<!-- TEMP: unique check context for formatter repair. -->
