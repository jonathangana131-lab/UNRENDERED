...
- `familyVersion` is a canonical positive signed-32-bit revision counter in `[1, 2147483647]`. Published canonical recipe changes advance this counter; values outside that bounded integer domain are rejected before StableId construction so the durable revision field cannot enter a numeric range whose increment/serialization semantics may diverge across future persistence or transport boundaries.
