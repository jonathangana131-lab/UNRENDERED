...
**Studio lifecycle driver:** the server bootstrap now retains the representation-safe `RealizedLab` handle through `PhysicsLabStudioHarness`. Use the server Command Bar procedures in `Docs/PHYSICS_LAB_STUDIO_RUNBOOK.md` to drive `lab.step`, teardown/rebuild, and 20-cycle snapshots through the existing production Realizer/runtime boundary. Do not require the raw `PhysicsLabRuntime`, mutate Instance attributes, or manually delete representations as lifecycle evidence. Lifecycle rows remain **UNVERIFIED** until those Studio procedures are actually run and recorded.
...
Exercise at least one promotion and one demotion through `PhysicsLabStudioHarness`, which exposes the representation-aware `RealizedLab` without exposing the raw runtime. Capture:
...
A local lab-only fidelity state machine is an architecture failure even if the visual transition looks correct. The harness existing does not prove fidelity behavior; leave this row UNVERIFIED until the runbook procedure is actually executed and its observations are recorded.
...
Using `PhysicsLabStudioHarness`, capture snapshots after initial settle, after a complete rebuild, and after rebuild cycles 1, 5, 10, and 20 as specified in `Docs/PHYSICS_LAB_STUDIO_RUNBOOK.md`. A partially demoted lab is intentionally a different physical representation, so record its count delta but do not apply the **full-lab envelope** assertion until the complete F2 lab has been rebuilt.
