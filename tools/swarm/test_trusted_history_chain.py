...
class TrustedHistoryChainTests(unittest.TestCase):
    def valid_event(self, summary: str) -> dict:
        return {
            "schemaVersion": 1,
            "eventId": "evt-20260811-214000-chain-regression",
            "timestamp": NOW,
    ...
    def test_restored_descendant_cannot_launder_invalid_middle_commit(self):
        ...
        try:
            ...
            # Endpoint-only validation demonstrates the laundering shape: A -> A passes.
            self.assertEqual(hard.transition_check(trusted.root, restored.root)["status"], "PASS")
            # Consecutive replay must encounter A -> B and fail before the restored tip is trusted.
            with self.assertRaises(core.ControlError):
                chain.validate_snapshot_chain([trusted.root, invalid.root, restored.root])
        finally:
            trusted.close()
            invalid.close()
            restored.close()
    ...
    def test_add_then_delete_cannot_disappear_between_trusted_endpoints(self):
        ...
        try:
            ...
            event = self.valid_event("transient append")
            event["eventId"] = "evt-20260811-214100-chain-transient"
            write(added.root / event_path, event)

            self.assertEqual(hard.transition_check(trusted.root, deleted.root)["status"], "PASS")
            with self.assertRaises(core.ControlError):
                chain.validate_snapshot_chain([trusted.root, added.root, deleted.root])
        finally:
            trusted.close()
            added.close()
            deleted.close()
    ...
    def test_first_parent_commit_order_is_deterministic(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            trusted_sha = commit("trusted")
            middle_sha = commit("middle")
            control_sha = commit("control")
            self.assertEqual(
                chain.first_parent_commits(repo, trusted_sha, control_sha),
                [middle_sha, control_sha],
            )
    ...
    def test_workflow_replays_chain_and_uses_trusted_base_primitives(self):
        workflow = (
            Path(__file__).resolve().parents[2] / ".github" / "workflows" / "swarm-control.yml"
        ).read_text(encoding="utf-8")
        self.assertIn("test_trusted_history_chain.py", workflow)
        self.assertIn("trusted_history_chain.py", workflow)
        self.assertIn('--trusted-sha "$TRUSTED_CONTROL_SHA"', workflow)
        self.assertIn('--control-sha "$CONTROL_SHA"', workflow)
        self.assertIn("malformed_event_quarantine_rows", workflow)
        self.assertIn("FIRST_WRITE_FILENAME", workflow)

        pr_ownership = workflow.split("  pr-ownership:\n", 1)[1].split("  validate-control-branch:\n", 1)[0]
        self.assertIn("hard.state_digest", pr_ownership)
        self.assertNotIn("hard.verify_trusted_state", pr_ownership)
        self.assertIn("bootstrap/reset mode", pr_ownership)
