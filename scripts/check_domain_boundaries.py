...
def _mask_luau(source: str, *, mask_strings: bool) -> str:
    """Mask comments and optionally string literal chunks, preserving offsets/newlines.

    Backtick interpolation is special: literal chunks are strings, while `{...}` contains
    executable Luau and must remain visible to code-only rules. Interpolation expressions
    may themselves contain nested tables, strings, comments, or interpolated strings.
    """
...
def _rules(path: Path, source: str) -> set[str]:
    return {rule for _, rule, _ in _find_violations(path, source)}
...
def _self_test() -> None:
    ...
    for path, source, expected_rule in cases:
        actual_rules = _rules(path, source)
...
--[=[ MemoryStoreService MessagingService Instance.new("Part") ]=]
local quoted = "workspace game:GetService('Players') MemoryStoreService"
local longQuoted = [=[Instance.new("Part") workspace DataStoreService]=]
local backtickText = `Instance.new('Part') MessagingService workspace`
local escapedInterpolation = `\{Instance.new('Part') workspace}`
...
'''.lstrip()
    ...
    interpolated_source = r'''
local literal = `Instance.new('Part') workspace game:GetService('Players')`
local instanceValue = `value = {Instance.new('Part')}`
local serviceValue = `value = {game:GetService('Players')}`
local workspaceValue = `value = {({ nested = { target = workspace.Part } }).nested.target}`
local nestedBacktick = `value = {`inner = {MessagingService}`}`
local commentInside = `value = { -- DataStoreService is a comment
    "MemoryStoreService is a string"
}`
'''.lstrip()
    interpolated = _find_violations(root / "src/shared/Core/Test.luau", interpolated_source)
    actual = [(line, rule) for line, rule, _ in interpolated]
    expected = [
        (2, "Roblox Instance construction in shared domain"),
        (3, "Roblox service access in shared domain"),
        (4, "Workspace access in shared domain"),
        (5, "MessagingService access in shared domain"),
    ]
    if actual != expected:
        raise AssertionError(
            f"domain-boundary interpolation self-test expected {expected!r}, got {actual!r}"
        )

    asset_comment = r'''
-- rbxassetid://123 is documentation only
--[=[ rbxassetid://456 is also documentation only ]=]
...
'''.lstrip()
    ...
    asset_backtick = "local texture = `rbxassetid://789`"
    if "embedded Roblox asset id in material/object domain" not in _rules(
        root / "src/shared/Objects/Test.luau", asset_backtick
    ):
        raise AssertionError("domain-boundary self-test missed asset id inside backtick literal")
    ...
    physics_scope = root / "src/shared/Physics/Test.luau"
    if "Workspace access in shared domain" not in _rules(
        physics_scope, "local physical = workspace.Physical"
    ):
        raise AssertionError("domain-boundary self-test did not protect the shared Physics domain root")
...
def main() -> int:
    ...
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="verify code-only, interpolation, and asset-literal boundary checks before auditing the repository",
    )
