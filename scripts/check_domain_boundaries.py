...
CODE_RULES = (
    Rule(
        "Roblox Instance construction in shared domain",
        re.compile(r"\bInstance\s*\.\s*new\s*\("),
    ),
    Rule(
        "Roblox service access in shared domain",
        re.compile(r"\bgame\s*:\s*GetService\s*\("),
    ),
    Rule(
        "Workspace access in shared domain",
        re.compile(r"\bworkspace\b"),
    ),
    Rule(
        "DataStoreService access in shared domain",
        re.compile(r"\bDataStoreService\b"),
    ),
    Rule(
        "MemoryStoreService access in shared domain",
        re.compile(r"\bMemoryStoreService\b"),
    ),
    Rule(
        "MessagingService access in shared domain",
        re.compile(r"\bMessagingService\b"),
    ),
)
ASSET_LITERAL = re.compile(r"rbxassetid://[0-9]+", re.IGNORECASE)
...
def _mask_luau(source: str, *, mask_strings: bool) -> str:
    """Mask comments and optionally literal string chunks while preserving interpolation code."""
    ...
    def mask_long_bracket(start: int, *, force_mask: bool = False) -> int | None:
        long_string = _long_bracket_level(source, start)
        if long_string is None:
            return None

        should_mask = force_mask or mask_strings
    ...
    def mask_comment(start: int) -> int:
        ...
        long_end = mask_long_bracket(cursor, force_mask=True)
    ...
    if _is_asset_literal_domain(path):
        ...
        for match in ASSET_LITERAL.finditer(comments_removed):
            ...
            violations.append(
                (
                    line_number,
                    "embedded Roblox asset id in material/object domain",
                    snippet,
                )
            )
...
def _self_test() -> None:
    ...
    cases = (
        (
            root / "src/shared/Objects/Test.luau",
            "local part = Instance.new('Part')",
            "Roblox Instance construction in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local players = game:GetService('Players')",
            "Roblox service access in shared domain",
        ),
        (
            root / "src/shared/Spatial/Test.luau",
        ...
        (
            root / "src/shared/Reality/Test.luau",
            "local service = MemoryStoreService",
            "MemoryStoreService access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local service = MessagingService",
            "MessagingService access in shared domain",
        ),
        (
            root / "src/shared/Materials/Test.luau",
            "local texture = 'rbxassetid://123'",
            "embedded Roblox asset id in material/object domain",
        ),
    )
    ...
    for path, source, expected_rule in cases:
        ...
        if expected_rule not in actual_rules:
            raise AssertionError(
                f"domain-boundary self-test expected {expected_rule!r}, got {sorted(actual_rules)!r}"
            )
...
local escapedInterpolation = `\{Instance.new('Part') is still literal text}`
local nestedQuoted = `value = {"workspace Instance.new('Part')"}`
local nestedLongQuoted = `value = {[[game:GetService('Players')]]}`
local longQuoted = [[workspace DataStoreService]]
...
local direct = `part = {Instance.new('Part')}`
local service = `players = {game:GetService('Players')}`
local nested = `part = {({ holder = { value = Instance.new('Part') } }).holder.value}`
local nestedBacktick = `outer = {`inner = {workspace.Model}`}`
'''.lstrip()
    interpolation_violations = _find_violations(
        root / "src/shared/Reality/Test.luau", interpolation_source
    )
    interpolation_actual = [(line, rule) for line, rule, _ in interpolation_violations]
    interpolation_expected = [
        (1, "Roblox Instance construction in shared domain"),
        (3, "Roblox Instance construction in shared domain"),
        (2, "Roblox service access in shared domain"),
        (4, "Workspace access in shared domain"),
    ]
    if interpolation_actual != interpolation_expected:
        raise AssertionError(
            "domain-boundary interpolation self-test failed: "
            f"expected {interpolation_expected!r}, got {interpolation_actual!r}"
        )

    asset_comment = "-- rbxassetid://123 is documentation only\nlocal family = 'commercial.carpet.lowpile'"
    asset_comment_violations = _find_violations(
        root / "src/shared/Materials/Test.luau", asset_comment
    )
    if asset_comment_violations:
        raise AssertionError(
            f"domain-boundary self-test flagged comment-only asset id: {asset_comment_violations!r}"
        )

    bare_asset_scheme = "local blockedPrefix = 'rbxassetid://'"
    bare_asset_scheme_violations = _find_violations(
        root / "src/shared/Materials/Test.luau", bare_asset_scheme
    )
    if bare_asset_scheme_violations:
        raise AssertionError(
            "domain-boundary self-test flagged bare asset scheme used by validation code: "
            f"{bare_asset_scheme_violations!r}"
        )

    asset_interpolation_comment = (
        "local diagnostic = `value = {1 -- rbxassetid://123 is comment-only\n}`"
    )
    asset_interpolation_comment_violations = _find_violations(
        root / "src/shared/Materials/Test.luau", asset_interpolation_comment
    )
    if asset_interpolation_comment_violations:
        raise AssertionError(
    ...
    outside_asset_domain = _find_violations(
        root / "src/shared/Core/Test.luau", "local diagnostic = 'rbxassetid://123'"
    )
    if outside_asset_domain:
        raise AssertionError(
            f"domain-boundary self-test applied asset rule outside its scope: {outside_asset_domain!r}"
        )

    physics_violation = _find_violations(
        root / "src/shared/Physics/Test.luau", "local model = workspace.Model"
    )
    if not physics_violation or physics_violation[0][1] != "Workspace access in shared domain":
        raise AssertionError(
            f"domain-boundary self-test did not protect Foundation Physics domain: {physics_violation!r}"
        )
...
def _audit_repository() -> int:
    ...
    if violations:
        ...
        print(
            "Keep Roblox representation, service, persistence, and concrete asset bindings behind project adapters.",
            file=sys.stderr,
        )
        return 1
...
def main() -> int:
    ...
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="verify code-only and asset-literal boundary checks before auditing the repository",
    )
