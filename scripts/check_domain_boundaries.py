...
def _mask_noncode(source: str) -> str:
    """Mask comments/string literal chunks while preserving interpolation expressions."""
...
def _mask_comments(source: str) -> str:
    """Mask Luau comments while preserving all string literal text and source offsets."""
    ...
    def skip_long_string(start: int) -> int | None:
        long_string = _long_bracket_level(source, start)
        if long_string is None:
            return None
        level, content_start = long_string
        close = "]" + ("=" * level) + "]"
        close_start = source.find(close, content_start)
        return length if close_start < 0 else close_start + len(close)
    ...
    def scan_backtick(start: int) -> int:
        cursor = start + 1
        while cursor < length:
            current = source[cursor]
            if current == "\\" and cursor + 1 < length:
    ...
    def scan_interpolation(start: int) -> int:
        ...
        while cursor < length:
            ...
            if current in ('"', "'"):
                ...
                continue
            if current == "[":
                long_end = skip_long_string(cursor)
                if long_end is not None:
                    cursor = long_end
                    continue
            if current == "`":
                cursor = scan_backtick(cursor)
                continue
            if current == "{":
                depth += 1
                cursor += 1
...
def _find_violations(path: Path, source: str) -> list[tuple[int, str, str]]:
    ...
    code_only = _mask_noncode(source)
    ...
    if _is_asset_literal_domain(path):
        comments_removed = _mask_comments(source)
...
local interpolatedLiteral = `Instance.new('Part') MessagingService`
...
local world = `value = {workspace.Model}`
local nested = `value = {({ part = Instance.new('Part') }).part}`
local nestedBacktick = `value = {`inner = {game:GetService('Players')}`}`
'''.lstrip()
    interpolation_violations = _find_violations(
        root / "src/shared/Reality/Test.luau", interpolation_source
    )
    actual_interpolation = [(line, rule) for line, rule, _ in interpolation_violations]
    expected_interpolation = [
        (1, "Roblox Instance construction in shared domain"),
        (4, "Roblox Instance construction in shared domain"),
        (2, "Roblox service access in shared domain"),
        (5, "Roblox service access in shared domain"),
        (3, "Workspace access in shared domain"),
    ]
    if actual_interpolation != expected_interpolation:
        raise AssertionError(
            "domain-boundary interpolation self-test failed: "
            f"expected {expected_interpolation!r}, got {actual_interpolation!r}"
        )
    ...
    interpolation_asset_comment = r'''
local value = `asset = {-- rbxassetid://123 is still a comment
    "commercial.carpet.lowpile"}`
'''.lstrip()
    interpolation_asset_violations = _find_violations(
        root / "src/shared/Materials/Test.luau", interpolation_asset_comment
    )
    if interpolation_asset_violations:
        raise AssertionError(
            "domain-boundary self-test flagged interpolation-comment asset id: "
            f"{interpolation_asset_violations!r}"
        )
