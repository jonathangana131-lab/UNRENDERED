...
def _mask_luau(source: str, *, mask_strings: bool) -> str:
    ...
    Luau backtick strings are not ordinary strings: their ``{...}`` regions execute code.
    In code-only mode we therefore mask only literal chunks and recursively scan those
    interpolation expressions. In comments-only mode we preserve string text so concrete
    asset-id literals can still be detected, while comments inside interpolations remain
    excluded.
    """
...
def _self_test() -> None:
    ...
    cases = (
        ...
        (
            root / "src/shared/Objects/Test.luau",
            "local text = `part = {Instance.new('Part')}`",
            "Roblox Instance construction in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `players = {game:GetService('Players')}`",
            "Roblox service access in shared domain",
        ),
        (
            root / "src/shared/Spatial/Test.luau",
            "local text = `model = {workspace.Model}`",
            "Workspace access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `service = {MemoryStoreService}`",
            "MemoryStoreService access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `outer = {({ value = `inner = {MessagingService}` }).value}`",
            "MessagingService access in shared domain",
        ),
    )
...
local escapedInterpolation = `\{workspace.Model}`
local interpolatedQuoted = `value = {"workspace DataStoreService"}`
...
'''.lstrip()
    ...
    asset_in_interpolated_comment = "local text = `value = {-- rbxassetid://123\n42}`"
    interpolation_comment_violations = _find_violations(
        root / "src/shared/Materials/Test.luau", asset_in_interpolated_comment
    )
    if interpolation_comment_violations:
        raise AssertionError(
            "domain-boundary self-test flagged asset id inside interpolation comment: "
            f"{interpolation_comment_violations!r}"
        )
