...
def _mask_luau(source: str, *, mask_strings: bool) -> str:
    ...
    def maybe_blank(index: int) -> None:
        if mask_strings:
            blank(index)
    ...
    def scan_long_string(start: int) -> int | None:
    ...
    def scan_interpolated(start: int) -> int:
        maybe_blank(start)
        cursor = start + 1
        while cursor < length:
            current = source[cursor]
    ...
    def scan_interpolation(start: int) -> int:
        ...
        while cursor < length:
            ...
            if current == "[":
                long_end = scan_long_string(cursor)
                if long_end is not None:
                    cursor = long_end
                    continue

            if current == "`":
                cursor = scan_interpolated(cursor)
                continue
...
def _self_test() -> None:
    ...
    cases = (
        ...
        (
            root / "src/shared/Objects/Test.luau",
            "local text = `value = {Instance.new('Part')}`",
            "Roblox Instance construction in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `value = {game:GetService('Players')}`",
            "Roblox service access in shared domain",
        ),
        (
            root / "src/shared/Spatial/Test.luau",
            "local text = `value = {workspace.Part}`",
            "Workspace access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `value = {({ nested = { service = DataStoreService } }).nested.service}`",
            "DataStoreService access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `outer = {`inner = {MemoryStoreService}`}`",
            "MemoryStoreService access in shared domain",
        ),
    )
    ...
    interpolation_lines = r'''
local literal = `Instance.new('Part') workspace MessagingService`
local escaped = `\{game:GetService('Players') is literal}`
local instance = `value = {Instance.new('Part')}`
local service = `value = {game:GetService('Players')}`
local nested = `value = {({ x = { y = 1 }, z = workspace }).z}`
local nestedBacktick = `value = {`inner = {MessagingService}`}`
'''.lstrip()
    interpolation_violations = _find_violations(
        root / "src/shared/Reality/Test.luau", interpolation_lines
    )
    actual_lines = [line for line, _, _ in interpolation_violations]
    expected_lines = [3, 4, 5, 6]
    if actual_lines != expected_lines:
        raise AssertionError(
            f"interpolation self-test expected violations on {expected_lines}, got {actual_lines}"
        )
...
local longQuoted = [[workspace DataStoreService Instance.new("Part")]]
local backtickLiteral = `Instance.new('Part') MessagingService workspace`
...
--[=[ rbxassetid://456 is also comment-only ]=]
local family = 'commercial.carpet.lowpile'
local diagnostic = `value = {-- rbxassetid://789\n42}`
'''.lstrip()
