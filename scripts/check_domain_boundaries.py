...
def _mask_luau(source: str, *, mask_strings: bool) -> str:
    """Mask comments and optional literal text while preserving interpolation code."""
...
def _self_test() -> None:
    ...
    cases = (
        ...
        (
            root / "src/shared/Spatial/Test.luau",
            "local text = `value = {workspace.Model}`",
            "Workspace access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `value = {({ service = MemoryStoreService }).service}`",
            "MemoryStoreService access in shared domain",
        ),
        (
            root / "src/shared/Reality/Test.luau",
            "local text = `outer = {`inner = {MessagingService}`}`",
            "MessagingService access in shared domain",
        ),
    )
...
--[=[ MemoryStoreService MessagingService workspace ]=]
...
local escapedBrace = `\{Instance.new('Part') is still literal text}`
local nestedStrings = `value = {("workspace" .. [[DataStoreService]])}`
...
local text = `value = {-- rbxassetid://789 comment inside interpolation
    "clean"
}`
'''.lstrip()
