...
def _mask_luau(source: str, *, mask_strings: bool) -> str:
    """Mask comments and optionally string literal text, preserving interpolation code."""
    ...
    def scan_long_bracket(start: int) -> int | None:
    ...
    def scan_interpolation(start: int) -> int:
        ...
        while cursor < length:
            ...
            if current == "[":
                long_end = scan_long_bracket(cursor)
...
local escaped = `\{Instance.new('Part') is literal text}`
local direct = `value = {Instance.new('Part')}`
local service = `value = {game:GetService('Players')}`
local workspaceValue = `value = {workspace.Part}`
local nestedTable = `value = {({ item = { model = Instance.new('Part') } }).item}`
local nestedBacktick = `value = {`inner = {MessagingService}`}`
""".lstrip()
    interpolation_path = root / "src/shared/Objects/Test.luau"
    interpolation_violations = _find_violations(interpolation_path, interpolation_source)
    actual = [(line, rule) for line, rule, _ in interpolation_violations]
    expected = [
        ...
        (6, "Roblox Instance construction in shared domain"),
        (7, "MessagingService access in shared domain"),
    ]
    if actual != expected:
        raise AssertionError(
            f"interpolation self-test expected {expected!r}, got {actual!r}"
        )
...
local escapedInterpolation = `\{game:GetService('Players') is literal text}`
...
--[=[ rbxassetid://222 is also documentation only ]=]
local quoted = "rbxassetid://333"
local longQuoted = [=[rbxassetid://444]=]
local backtickText = `asset rbxassetid://555`
local interpolation = `asset = {"rbxassetid://666"}`
""".lstrip()
    asset_violations = _find_violations(root / "src/shared/Materials/Test.luau", asset_source)
    asset_lines = [
        line
        for line, rule, _ in asset_violations
        if rule == "embedded Roblox asset id in material/object domain"
    ]
    if asset_lines != [3, 4, 5, 6]:
        raise AssertionError(
            f"asset-literal self-test expected lines [3, 4, 5, 6], got {asset_lines!r}"
        )
